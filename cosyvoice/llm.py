...
class Qwen2LM(TransformerLM):
    ...
    @torch.inference_mode()
    def inference_vllm_bistream_original(
        self,
        text: Generator,
        prompt_text: torch.Tensor,
        prompt_text_len: torch.Tensor,
        ...
        for this_text in text:
            ...
            print("text_cache.shape, prompt_speech_token_emb.shape: ", text_cache.shape, prompt_speech_token_emb.shape)
            
            while True:
                if text_cache.size(1) >= self.mix_ratio[0] and prompt_speech_token_emb.size(1) >= self.mix_ratio[1]:
            ...
            if prompt_speech_token_emb.size(1) < self.mix_ratio[1]:

                if text_cache.size(1) >= self.mix_ratio[0]:
                    lm_input_text = text_cache[:, :self.mix_ratio[0]]
                    logging.info('append {} text token'.format(lm_input_text.size(1)))
 
                    lm_input = torch.concat([lm_input, lm_input_text, prompt_speech_token_emb], dim=1)
            ...
            for speech_token in self.inference_wrapper(lm_input, sampling, 0, self.mix_ratio[1], uuid):
        ...
        for speech_token in self.inference_wrapper(lm_input, sampling, 0, int((max_token_text_ratio - self.mix_ratio[1] / self.mix_ratio[0]) * text_len), uuid):
            prompt_speech_token_emb = torch.concat([prompt_speech_token_emb, self.speech_embedding.weight[speech_token].reshape(1, 1, -1)], dim=1)
            yield speech_token
