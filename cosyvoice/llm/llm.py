...
class Qwen2LM(TransformerLM):
    ...
    def inference_vllm_bistream(
        ...
        device = prompt_text.device
        # 1. prepare input
        sos_eos_emb = self.llm_embedding.weight[self.sos].reshape(1, 1, -1)
        task_id_emb = self.llm_embedding.weight[self.task_id].reshape(1, 1, -1)
        if prompt_speech_token_len != 0:
            prompt_speech_token_emb = self.speech_embedding(prompt_speech_token)
        else:
        ...
        for this_text in text:
            try:
                ...
                if text_len % 3 == 0:#长度长一点，增加识别语言的准确性
                    print(text_len, text_cache)
                ...
                print("INFERENCE WRONG!")
                break

        # 3. final decode
        try:
            print("final",text_cache)
            lm_input = torch.concat([sos_eos_emb, text_cache, task_id_emb, prompt_speech_token_emb], dim=1)
            logging.info('no more text token, decode until met eos')
...
class CosyVoice3LM(Qwen2LM):
    ...
    def inference_vllm_bistream(
        ...
        device = prompt_text.device
        # 1. prepare input
        sos_eos_emb = self.speech_embedding.weight[self.sos].reshape(1, 1, -1)
        task_id_emb = self.speech_embedding.weight[self.task_id].reshape(1, 1, -1)
        if prompt_speech_token_len != 0:
            prompt_speech_token_emb = self.speech_embedding(prompt_speech_token)
        else:
