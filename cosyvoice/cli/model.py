...
class CosyVoiceModel:
    ...
    def llm_job(self, text, prompt_text, llm_prompt_speech_token, llm_embedding, uuid, max_token_text_ratio=20, min_token_text_ratio=2):
    ...
    def tts(self, text=torch.zeros(1, 0, dtype=torch.int32), flow_embedding=torch.zeros(0, 192), llm_embedding=torch.zeros(0, 192),
        ...
        if source_speech_token.shape[1] == 0:
            max_token_text_ratio = kwargs.pop('max_token_text_ratio', None)
            min_token_text_ratio = kwargs.pop('min_token_text_ratio', None)
            print("max_token_text_ratio, min_token_text_ratio: ", max_token_text_ratio, min_token_text_ratio)
            
            if max_token_text_ratio is not None and min_token_text_ratio is not None:
                print("max_token_text_ratio, min_token_text_ratio: ", max_token_text_ratio, min_token_text_ratio)
                p = threading.Thread(target=self.llm_job, args=(text, prompt_text, llm_prompt_speech_token, llm_embedding, max_token_text_ratio, min_token_text_ratio, this_uuid))
            else:
...
class CosyVoice2Model(CosyVoiceModel):
    ...
    def tts(self, text=torch.zeros(1, 0, dtype=torch.int32), flow_embedding=torch.zeros(0, 192), llm_embedding=torch.zeros(0, 192),
        ...
        if source_speech_token.shape[1] == 0:
            ...
            if max_token_text_ratio is not None and min_token_text_ratio is not None:
                print("max_token_text_ratio, min_token_text_ratio: ", max_token_text_ratio, min_token_text_ratio)
                p = threading.Thread(target=self.llm_job, args=(text, prompt_text, llm_prompt_speech_token, llm_embedding, this_uuid, max_token_text_ratio, min_token_text_ratio))
            else:
                p = threading.Thread(target=self.llm_job, args=(text, prompt_text, llm_prompt_speech_token, llm_embedding, this_uuid))
        else:
            p = threading.Thread(target=self.vc_job, args=(source_speech_token, this_uuid))
            ...
            while True:
                ...
                if len(self.tts_speech_token_dict[this_uuid]) - token_offset >= this_token_hop_len + self.flow.pre_lookahead_len:
                    ...
                    this_tts_speech = self.token2wav(token=this_tts_speech_token,
                                                     ...
                                                     stream=stream,
                                                     finalize=False)
                    print("token2wav",time.time()-start)
