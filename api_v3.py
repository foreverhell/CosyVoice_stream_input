...
parser.add_argument('--model', type=str, default='pretrained_models/Fun-CosyVoice3-0.5B')#"../CosyVoice_stream_input/pretrained_models/CosyVoice2-0.5B"
#server
...
parser.add_argument('--version', type=str, default="v3", help="version")#omni的版本号
...
def warmup(times=2):
    prompt_speech_16k = './asset/zero_shot_prompt.wav' #load_wav('./asset/zero_shot_prompt.wav', 16000)
...
def inference(mode, spk_id, text, ref_text, prompt_speech_16k, stream=True, openai=False, modalities = ["audio"], lang="zh"):
    ...
    if mode=="sft":
        ...
        if isinstance(text,str):
            for _, j in enumerate(cosyvoice.inference_zero_shot(text, "You are a helpful assistant.<|endofprompt|>"+ref_text, prompt_speech_16k, zero_shot_spk_id="", stream=stream, lang=lang)):
                output = j["tts_speech"]
                audio_stream = (output.numpy() * 32767).astype(np.int16).tobytes()
                yield audio_stream
